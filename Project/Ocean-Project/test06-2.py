...
#로지스틱 회귀모형에 피팅 (NaN행 무시), type을 float로 바꿔준다?
...
data = {'Observation Date':data_frame['Observation Date'], 'Phosphate Phosphorus': Phosphate_list, 'Nitrous Acid Nitrogen':Nitrous_list, 'Nitric Acid Nitrogen':Nitric_list, 'Silicic Acid Silicon':Silicic_list, 'Probability':at_means_list}
df = pd.DataFrame(data)

#Nan행 삭제
data_edit = df.dropna()

data_edit.to_csv('result03-2.csv', index=False)
...
#header=0은 해더행 무시한다는 의미. 데이터프레임으로 읽음
data_frame = pd.read_csv('result03-2.csv', header=0)
#독립변수로 선언
independent_variables = data_frame[['Phosphate Phosphorus','Nitrous Acid Nitrogen','Nitric Acid Nitrogen','Silicic Acid Silicon']]
