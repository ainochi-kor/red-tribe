...
for i in range(len(data_frame['Observation Date'])):
    ...
    #소수점 3째 자리까지 반올림
    at_means_list.append(round(((inverse_logit(at_means))*100), 4))

data = {'Observation Date':data_frame['Observation Date'], 'Temperature': temp_list, 'Salinity':sal_list, 'Dissolved Oxygen':oxy_list, 'Probability':at_means_list}
df = pd.DataFrame(data)

#Nan행 삭제
data_edit = df.dropna()

data_edit.to_csv('result03-1.csv', index=False)
...
#header=0은 해더행 무시한다는 의미. 데이터프레임으로 읽음
data_frame = pd.read_csv('result03-1.csv', header=0)
#독립변수로 선언
independent_variables = data_frame[['Temperature', 'Salinity', 'Dissolved Oxygen']]
